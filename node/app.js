...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('chowner', function(data) {
        ...
        fs.readFile('json/listroom.json', function(err, data) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                if ((json[i].roomId == socket.room) && (json[i].pass == pass) || (json[i].pass == null)) {
                    json[i].owner = socket.id;
                    if(json[i].pass==null){
                      json[i].pass = pass;
                    }
                    
                    console.log("json[i]"+json[i].pass);
                    //together
                    setTimeout(function() {
                    ...
                    fs.writeFile('json/listroom.json', JSON.stringify(json), function(err) {
                        ...
                        socket.emit("alert","password correct");
        ...
        fs.readFile('json/listroom.json', function(err, data) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                if ((json[i].roomId == socket.room) && (json[i].owner == socket.id && json[i].playnow != null)) {
                    ...
                    console.log("together : " + socket.id);
        ...
        fs.readFile('json/listroom.json', function(err, data) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                if ((json[i].roomId == socket.room) && (json[i].owner == socket.id)) {
                    ...
                    socket.broadcast.to(socket.room).emit('stop', json[i]);
                } else {
    ...
    socket.on('play', function(data) {
        ...
        fs.readFile('json/listroom.json', function(err, data) {
            ...
            for (var i = 0; i < json.length; i++) {
                if ((json[i].roomId == socket.room) && (json[i].owner == socket.id)) {
                    console.log(socket.id + " onPlay : User OK >> " + json[i]);
                    socket.broadcast.to(socket.room).emit('play', json[i]);
                } else {
